...
//Timer variables
long runForCheck = 0;
long runFor = 0; //Run air for this much time
//Countdown var
int countDown = 5;
...
void toggleButton1(int inputPin, int toggledItem) {
  int toggleState;
  toggleState = digitalRead(inputPin);
  
  if (toggleState == LOW  && timeWait(timer) )
  {
    digitalWrite(toggledItem, LOW);
    timer = millis();
    countVar = count(countVar, 1);
  } 
  else if (toggleState == HIGH && timeWait(timer) ) 
  {
...
//__________________________ignore the below_________________________________
